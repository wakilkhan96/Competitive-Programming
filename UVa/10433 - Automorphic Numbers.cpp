...
int main()
{
    ...
    while(cin>>str)
    {
        ...
        string ans = productString_and_String(str, str);
        if(ans.length()<str.length() || str=="0" || str=="1")
        {
        ...
        else
        {
            ...
            for(int x=0; str[x]; x++)
            {
                if(str[x]!=ans[x])isAuto = false;
            }
            if(isAuto) printf("Automorphic number of %d-digit.\n", str.length());
